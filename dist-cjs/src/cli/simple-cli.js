            ...
            switch(terminalCmd){
                ...
                case 'exec':
                    ...
                    if (execCmd && sessionFlag < 0) {
                        ...
                        const command = cmdStart >= 0 && cmdEnd > cmdStart ? subArgs.slice(cmdStart, cmdEnd + 1).join(' ').slice(1, -1) : 'echo "No command"';
        ...
        case 'project':
            ...
            switch(projectCmd){
                ...
                case 'backup':
                    ...
                    const outputFlag1 = subArgs.indexOf('--output');
                    ...
                    const outputFile = outputFlag1 >= 0 ? subArgs[outputFlag1 + 1] : `${backupProject}-backup-${Date.now()}.tar.gz`;
            ...
            switch(claudeCmd){
                case 'spawn':
                    ...
                    for(let i = taskEndIndex; i < subArgs.length; i++){
                        const arg = subArgs[i];
                        if (arg === '--tools' || arg === '-t') {
                            flags1.tools = subArgs[++i];
                        } else if (arg === '--no-permissions') {
                            flags1.noPermissions = true;
                        } else if (arg === '--config' || arg === '-c') {
                            flags1.config = subArgs[++i];
                        } else if (arg === '--mode' || arg === '-m') {
                            flags1.mode = subArgs[++i];
                        } else if (arg === '--parallel') {
                            flags1.parallel = true;
                        } else if (arg === '--research') {
                            flags1.research = true;
                        } else if (arg === '--coverage') {
                            flags1.coverage = parseInt(subArgs[++i]);
                        } else if (arg === '--commit') {
                            flags1.commit = subArgs[++i];
                    ...
                    let tools = flags1.tools || 'View,Edit,Replace,GlobTool,GrepTool,LS,Bash';
                    if (flags1.parallel) {
                    ...
                    if (flags1.dryRun) {
                        ...
                        console.log(`Mode: ${flags1.mode || 'full'}`);
                        console.log(`Coverage: ${flags1.coverage || 80}%`);
                        console.log(`Commit: ${flags1.commit || 'phase'}`);
                        console.log(`\nEnhanced Features:`);
                        console.log(`  - Memory Bank enabled via: npx claude-flow memory commands`);
                        console.log(`  - Coordination ${flags1.parallel ? 'enabled' : 'disabled'}`);
                        console.log(`  - Access Claude-Flow features through Bash tool`);
                    } else {
                        printSuccess(`Spawning Claude instance: ${instanceId}`);
                        console.log(`📝 Original Task: ${task}`);
                        console.log(`🔧 Tools: ${tools}`);
                        console.log(`⚙️  Mode: ${flags1.mode || 'full'}`);
                        console.log(`📊 Coverage: ${flags1.coverage || 80}%`);
                        console.log(`💾 Commit: ${flags1.commit || 'phase'}`);
...
- Development Mode: ${flags1.mode || 'full'}
${flags1.coverage ? `- Test Coverage Target: ${flags1.coverage}%` : ''}
${flags1.commit ? `- Git Commit Strategy: ${flags1.commit}` : ''}
${flags1.config ? `- MCP Config: ${flags1.config}` : ''}

### Available Tools
- You have access to these tools: ${tools}
${flags1.tools ? `- Custom tools specified: ${flags1.tools}` : ''}
...
1. **Focus on Your Primary Objective**:
   ...
   ${flags1.mode === 'backend-only' ? '   - Focus on backend implementation' : ''}
   ${flags1.mode === 'frontend-only' ? '   - Focus on frontend implementation' : ''}
   ${flags1.mode === 'api-only' ? '   - Focus on API design and implementation' : ''}

2. **Development Best Practices**:
   - Write clean, maintainable code following project conventions
   - Include appropriate tests and documentation
   - Use version control effectively
   ${flags1.coverage ? `   - Ensure test coverage meets ${flags1.coverage}% target` : ''}
   ${flags1.commit === 'phase' ? '   - Commit changes after completing major phases' : ''}
   ${flags1.commit === 'feature' ? '   - Commit changes after each feature is complete' : ''}
   ${flags1.commit === 'manual' ? '   - Only commit when explicitly requested' : ''}
...
3. **Leverage Orchestration When Beneficial**:
   ...
   ${flags1.parallel ? '   - Parallel capabilities are enabled for this task' : ''}
   ${flags1.research ? '   - Research tools are available if needed' : ''}
   ${flags1.noPermissions ? '   - Running with --no-permissions mode' : ''}
   ${flags1.verbose ? '   - Verbose mode enabled for detailed output' : ''}
...
## Development Mode: ${flags1.mode || 'full'}
${flags1.mode === 'backend-only' ? `Focus on server-side implementation, APIs, and business logic.` : ''}
${flags1.mode === 'frontend-only' ? `Focus on client-side implementation, UI/UX, and user interactions.` : ''}
${flags1.mode === 'api-only' ? `Focus on API design, documentation, and endpoint implementation.` : ''}
${flags1.mode === 'full' || !flags1.mode ? `Full-stack development covering all aspects of the application.` : ''}
...
`;
                        ...
                        if (flags1.noPermissions) {
                            claudeArgs.push('--dangerously-skip-permissions');
                        }
                        if (flags1.config) {
                            claudeArgs.push('--mcp-config', flags1.config);
                        }
                        if (flags1.verbose) {
                        ...
                        try {
                            ...
                            const child = spawn('claude', claudeArgs, {
                                env: {
                                    ...
                                    CLAUDE_FLOW_COMMIT: flags1.commit || 'phase',
                    ...
                    break;
                case 'batch':
                    const workflowFile1 = subArgs[1];
                    if (!workflowFile1) {
                        printError('Usage: claude batch <workflow-file>');
                        break;
                    }
                    printSuccess(`Loading workflow: ${workflowFile1}`);
