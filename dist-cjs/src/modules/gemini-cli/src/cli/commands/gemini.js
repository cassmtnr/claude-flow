...
export const geminiCommand = new Command().description('Manage Gemini CLI integration for large-scale codebase analysis').action(()=>{
...
}).command('enable', new Command().description('Enable and authenticate Gemini CLI').option('--auth <method:string>', 'Authentication method: google-login, api-key, vertex-ai', {
    default: 'google-login'
}).option('--api-key <key:string>', 'API key (for api-key auth)').option('--vertex-project <project:string>', 'GCP project ID (for vertex-ai auth)').option('--vertex-location <location:string>', 'GCP region (for vertex-ai auth)', {
    default: 'us-central1'
}).option('--skip-install', 'Skip Gemini CLI installation check').action(async (options)=>{
    try {
...
})).command('disable', new Command().description('Disable Gemini CLI temporarily').action(async ()=>{
    try {
        const module = getGeminiModule();
...
})).command('status', new Command().description('Show Gemini CLI module status').action(async ()=>{
    try {
        const module = getGeminiModule();
...
})).command('analyze', new Command().description('Run codebase analysis').option('-t, --type <type:string>', 'Analysis type: codebase, architecture, security, dependencies, coverage', {
    default: 'codebase'
}).option('-p, --path <paths:string[]>', 'Paths to analyze', {
    default: [
        '.'
    ]
}).option('-q, --query <query:string>', 'Custom analysis query').option('-o, --output <format:string>', 'Output format: json, markdown, text', {
    default: 'markdown'
}).option('-d, --depth <depth:string>', 'Analysis depth: surface, moderate, deep, comprehensive', {
    default: 'moderate'
}).option('--store-memory', 'Store results in Claude Flow memory').action(async (options)=>{
    try {
        const module = getGeminiModule();
        if (!module.isEnabled()) {
...
})).command('security', new Command().description('Run security scan').option('-p, --path <path:string>', 'Path to scan', {
    default: '.'
...
})).command('architecture', new Command().description('Map system architecture').option('-p, --path <path:string>', 'Path to analyze', {
    default: '.'
}).action(async (options)=>{
    try {
        const module = getGeminiModule();
...
})).command('verify', new Command().description('Verify if a feature is implemented').option("-f, --feature <description:string>", 'Feature to verify', {
    required: true
}).option('-p, --path <path:string>', 'Path to check', {
    default: '.'
}).action(async (options)=>{
    try {
        const module = getGeminiModule();
...
})).command('cache', new Command().description('Manage analysis cache').option('--clear', 'Clear all cached results').option('--stats', 'Show cache statistics').action(async (options)=>{
    try {
        const module = getGeminiModule();
        await module.initialize();
        if (options.clear) {
...
})).command('eject', new Command().description('Completely remove Gemini CLI module').option('--force', 'Skip confirmation').option('--uninstall', 'Also uninstall Gemini CLI globally').option('--keep-config', 'Keep configuration for future use').action(async (options)=>{
    try {
        if (!options.force) {
